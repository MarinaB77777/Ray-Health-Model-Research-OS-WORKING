...
# Operational memory classes defined by the current multi-memory architecture.
# These are identifiers for governed stores, not permission to access every record.
EXTERNAL_CORE_MEMORY_CLASSES = frozenset(
    {
        "working_operational",
        "episodic",
        "semantic",
...
# Raw protected compartments are never External Core memory classes.
FORBIDDEN_MEMORY_CLASSES = frozenset(
    {
        "heart_of_ray",
        "heart_of_human",
        "inner_core_raw",
...
FORBIDDEN_SETTING_KEYS = frozenset(
    {
        ...
        "inner_core_payload",
        "heart_of_ray_payload",
        "heart_of_human_payload",
        "raw_inner_core",
    }
)
...
class RaySettingsRevision:
    ...
    # Legacy operational purpose/context scope. Retained for backward compatibility.
    allowed_memory_scopes: tuple[str, ...] | None = None
    # Canonical store-class axis. This does not itself grant record-level access.
    allowed_memory_classes: tuple[str, ...] | None = None
    ...
    def validate(self, *, require_complete_default: bool = False) -> None:
        ...
        if self.allowed_memory_classes is not None:
            classes = set(self.allowed_memory_classes)
            if classes & FORBIDDEN_MEMORY_CLASSES:
                raise PermissionError("RAW_INNER_CORE_CANNOT_BE_EXTERNAL_MEMORY_CLASS")
            if not classes.issubset(EXTERNAL_CORE_MEMORY_CLASSES):
                raise ValueError("UNKNOWN_EXTERNAL_CORE_MEMORY_CLASS")
        ...
        if self.default_language is not None:
            ...
            if self.allowed_languages is not None and self.default_language not in self.allowed_languages:
...
class EffectiveRaySettings:
    ...
    def to_dict(self) -> dict[str, Any]:
        data = asdict(self)
        for key in ("clarification_policy", "uncertainty_detail", "external_ai_mode"):
    ...
    def resolve(
        ...
        *,
        ...
        effective: dict[str, Any] = {
            field_name: set(getattr(base, field_name) or ()) for field_name in self.SET_FIELDS
        }
        effective.update(
            {
                ...
                "human_confirmation_actions": set(base.human_confirmation_actions),
        ...
        provenance: dict[str, list[str]] = {
            field_name: [self._revision_ref(base)] for field_name in effective
        }
        ...
        for profile in profiles[1:]:
            ...
            if profile.maximum_retention_days is not None:
                effective["maximum_retention_days"] = min(
                    effective["maximum_retention_days"], profile.maximum_retention_days
                )
            ...
            effective["human_confirmation_actions"].update(profile.human_confirmation_actions)
            ...
            if profile.external_ai_mode is not None:
                ...
                if EXTERNAL_AI_AUTHORITY[profile.external_ai_mode] > EXTERNAL_AI_AUTHORITY[current]:
            ...
            if profile.uncertainty_detail is not None:
                ...
                if UNCERTAINTY_DETAIL_RANK[profile.uncertainty_detail] < UNCERTAINTY_DETAIL_RANK[current]:
        ...
        if effective["default_language"] not in effective["allowed_languages"]:
            effective["default_language"] = sorted(effective["allowed_languages"])[0]
        ...
        return EffectiveRaySettings(
            ...
            allowed_memory_scopes=tuple(sorted(effective["allowed_memory_scopes"])),
            allowed_memory_classes=tuple(sorted(effective["allowed_memory_classes"])),
            maximum_retention_days=effective["maximum_retention_days"],
            learning_enabled=effective["learning_enabled"],
            allowed_learning_categories=tuple(sorted(effective["allowed_learning_categories"])),
            human_confirmation_actions=tuple(sorted(effective["human_confirmation_actions"])),
            ...
            provenance={key: tuple(values) for key, values in provenance.items()},
        )
...
class RaySettingsRegistry:
    ...
    def save_draft(self, revision: RaySettingsRevision) -> dict[str, Any]:
        ...
        if existing:
            ...
            first = existing[0]
            if first["layer"] != revision.layer.value or first["scope_id"] != revision.scope_id:
    ...
    def transition(
        ...
        *,
        ...
        allowed = {SettingsStatus.DRAFT: SettingsStatus.TRIAL, SettingsStatus.TRIAL: SettingsStatus.ACTIVE}
        ...
        if target == SettingsStatus.ACTIVE:
            ...
            for revisions in state["revisions"].values():
                for candidate in revisions:
                    if candidate["layer"] == item["layer"] and candidate["scope_id"] == item["scope_id"]:
                        candidate["current"] = candidate is item
        self._validate_serialized(item)
        self._append_event(state, f"settings_transitioned_to_{target.value}", item, actor_id)
        state["schema_version"] = "2.0.0"
        self._write(state)
        return item.copy()

    def delete_draft(self, settings_id: str, revision: int, *, actor_id: str) -> None:
        state = self._load()
        item = self._find(state, settings_id, revision)
    ...
    def _serialize(revision: RaySettingsRevision) -> dict[str, Any]:
        ...
        for key in ("layer", "status", "clarification_policy", "uncertainty_detail", "external_ai_mode"):
    ...
    def _deserialize(data: dict[str, Any]) -> RaySettingsRevision:
        ...
        # Backward-compatible migration: old registries had no memory-class axis.
        # They receive only Working/Operational by default; broader classes require an explicit new revision.
        if "allowed_memory_classes" not in converted:
            converted["allowed_memory_classes"] = ["working_operational"]
    ...
    @staticmethod
    def _find(state: dict[str, Any], settings_id: str, revision: int) -> dict[str, Any]:
    ...
    @staticmethod
    def _append_event(state: dict[str, Any], event_type: str, item: dict[str, Any], actor_id: str) -> None:
    ...
    def _load(self) -> dict[str, Any]:
        if not self.path.exists():
            return {"schema_version": "2.0.0", "revisions": {}, "audit_events": []}
        data = json.loads(self.path.read_text(encoding="utf-8"))
        if not isinstance(data, dict) or not {"schema_version", "revisions", "audit_events"}.issubset(data):
